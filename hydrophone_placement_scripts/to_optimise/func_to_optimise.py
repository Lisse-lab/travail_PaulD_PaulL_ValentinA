...
###Construction of df_areas
    ...
    def calc_nav_weight(self, df_areas):
        ...
        for areat in l_nav:
            boolean = True
            i = 1 #we cannot calculate transmission loss when it is vertical so we assume it is 0
            df_areas.loc[((df_areas["x"] == areat[0]) & (df_areas["y"] == areat[1])), "nav_exp"] += excess_noise
            while boolean:
                j = 0
                while (j <= i) & boolean:
                    boolean = False
                    l = ut.comb(i,j, areat)
                    for u,v in l:
                        ds = df_areas["d"][(df_areas["x"] == u) & (df_areas["y"] == v)]
                        if len(ds) > 0:
                            depths, substrat = self.topo.depths_and_substrat(areat, (u,v))
                            for d in ds:
                                tloss = 0
                                for freq in self.boat_freqs:
                                    tloss = self.tloss(0, self.area_depth(u,v,d), freq, depths, substrat)
                                exp = max(excess_noise - tloss/len(self.boat_freqs), 0)
                                df_areas.loc[((df_areas["x"] == u) & (df_areas["y"] == v) & (df_areas["d"] == d)), "nav_exp"] += exp
                                if exp != 0:
                                    boolean = True
                    j+=1
                i+=1
        ...
        if (not new) & ("df_areas.csv" in os.listdir(self.path)):
            ...
            for i in range (self.converter.n_areas_x):
                for j in range(self.converter.n_areas_y):
                    d = self.topo.dic_depths[(i,j)]
                    if d>0:
                        d = np.ceil(d/self.converter.depth_area).astype(int)
                        for k in range (d):
                            df_areas.at[n, "x"] = i
                            df_areas.at[n, "y"] = j
                            df_areas.at[n, "d"] = k
...
### Acoustic losses
    ...
    def tloss(self, deptht, depthr, freq, depths, substrat):
        args={
            "frequency" : freq,
            "depth": depths,
        ...
        print(self.alpha(freq, depthr, sound_speed), self.alpha(freq, depthr, sound_speed)*depths[-2][0])
        print(-20 * ut.log10(np.abs(tloss)))
...
###Error :
    ...
    def calc_tdoa_errors_hydro(self, snr_power, k, freq, freq_range):
    ...
    def set_range(self, new = False):
        if (not new) & ("df_ranges.csv" in os.listdir(self.path)):
            ...
            df_ranges = pd.DataFrame(columns=["source_level_beluga", "freq", "freq_range_min", "freq_range_max", "noise", "range_mean", "range_var", "n_range"])
        ...
        if len(l_to_calculate) > 0:
            ...
            for freq_freq_range in l_to_calculate:    
                df_ranges.loc[len(df_ranges)] = (self.source_level_beluga, freq_freq_range[0], freq_freq_range[1][0], freq_freq_range[1][1], self.noises[freq], np.mean(dic_ranges[freq_freq_range[0]]), np.var(dic_ranges[freq_freq_range[0]], ddof=1), len(dic_ranges[freq_freq_range[0]]))
    ...
    def get_range(self, arg):
        freq, freq_range, k = arg
        point = cls_points.PointBayesian()
        area = self.converter.utm2area(point.coords[0], point.coords[1])
        i = 0
        boolean = True
        while boolean:
            ...
            while (j <= i) & boolean:
                ...
                for u,v in l:
                    ...
                    if len(ds) > 0:
                        ...
                        for d in ds:
                            snr_power = self.calc_snr_power(self.area_depth(u,v,d), point.depth(), freq, depths, substrat) if i != 0 else self.source_level_beluga - self.noises[freq]
                            _, use_crb = self.calc_tdoa_errors_hydro(snr_power, k, freq, freq_range)
                            if use_crb:
                                boolean = True
                j+=1
            i+=1
        if i==j:
            ...
            j-=1
        l = ut.comb(i,j,area)
        return freq, np.max([self.converter.dist_areas((u,v), area) for u,v in l])
