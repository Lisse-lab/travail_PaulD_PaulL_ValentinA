...
class Conv :
    ...
    def __init__(self, lat_min : float | None = None, lat_max : float | None = None, lon_min : float | None = None, lon_max : float | None = None, width_area : float | None = None, depth_area : float | None = None):
    ...
    def area2lla(self, area : tuple[int, int]):
        x, y = self.area2utm(area)
        return self.utm2lla(x, y)
    
    def area2perim_lla(self, area : tuple[int, int]):
    ...
    def lla2enu(self, lat : float, lon : float, alt : float=0):
    ...
    def enu2lla(self, x : float, y : float, z : float=0):
    ...
    def update_enu_ref(self, lat : float, lon : float, alt : float=0):
    ...
    def utm2enu(self, x : float, y : float, d : float=0): #d is depth so it is positive downward and and we need an altitude
        lat, lon = self.utm2lla(x, y)
        return self.lla2enu(lat, lon, -d)
    
    def enu2utm(self, x : float, y : float, z : float=0):
        lat, lon, a = self.enu2lla(x, y, z)
        x, y = self.lla2utm(lat, lon)
        return (x, y, -a)

    def area2enu(self, area : tuple[int, int], d : float=0):
        x, y = self.area2utm(area)
        return self.utm2enu(x, y, d)
    
    def dist_areas(self, area1 : tuple[int, int], area2 : tuple[int, int]):
